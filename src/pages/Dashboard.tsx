...
/** Compress image to max 1000px WebP at 80% quality */
const compressCoverImage = (file: File): Promise<File> => {
        ...
        {loading ? (
          ...
          <div data-tour="projects-list" className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {projects.map((project, i) => (
              <div
                key={project.id}
                className={`text-left bg-card border rounded-lg p-6 transition-all group animate-fade-in relative overflow-hidden cursor-pointer ${manageMode ? "border-primary/30" : "border-border hover:border-foreground/20 hover:shadow-lg hover:shadow-foreground/5 hover:-translate-y-0.5"}`}
