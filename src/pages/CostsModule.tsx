...
const CostsModule = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) { toast.error("Error al enviar la certificación"); setSubmitting(false); return; }
    await supabase.from("audit_logs").insert({ user_id: user.id, project_id: projectId, action: "cost_claim_submitted", details: { title, amount } });
    ...
    <AppLayout>
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-xs font-display uppercase tracking-[0.2em] text-muted-foreground">Validación de Costes</p>
        </div>
          ...
          {canSubmit && (
            <Dialog open={createOpen} onOpenChange={setCreateOpen}>
              <DialogTrigger asChild>
                <Button className="font-display text-xs uppercase tracking-wider gap-2"><Plus className="h-4 w-4" />Nueva Certificación</Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader><DialogTitle className="font-display">Enviar Certificación</DialogTitle></DialogHeader>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`px-2 py-0.5 text-[10px] font-display uppercase tracking-widest rounded ${st.color}`}>{st.label}</span>
                      </div>
                      ...
                      {claim.file_name && <span className="flex items-center gap-1 text-xs text-muted-foreground mt-1"><FileText className="h-3 w-3" /> {claim.file_name}</span>}
      ...
      <AlertDialog open={!!actionClaim} onOpenChange={() => { setActionClaim(null); setRejectReason(""); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="font-display">{actionClaim?.action === "reject" ? "Rechazar Certificación" : "Confirmar Acción"}</AlertDialogTitle>
